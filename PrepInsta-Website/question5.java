...
public class question5 {
    public static void main(String[] args) {
        ...
        System.out.println("Enter the guests entering: ");

        for(int i=0 ; i < entry.length ; i++){
            entry[i] = sc.nextInt() ; 
        }

        System.out.println("Enter the guests leaving: ");

        for(int i=0 ; i < leave.length ; i++){
            leave[i] = sc.nextInt() ; 
        }

        System.out.println(maxGuests(time, entry, leave)); ; 

    }
    
    public static int maxGuests(int time, int[] entry, int[] leave){

        int maximumGuests = 0, currentGuest = 0; 

        for(int i=0 ; i < time ; i++){

                currentGuest += leave[i] - entry[i] ; 

                if(currentGuest > maximumGuests){
                    maximumGuests = currentGuest ; 
                }
